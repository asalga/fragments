...
float rectSDF(vec2 p, vec2 dims){
  vec2 a = abs(p / dims);
  float v = max(a.x, a.y);
  float test = smoothstep(v, v+e,  dims.x) + smoothstep(v, v+e,  dims.y);
  //ew ew ew, need to learn a better way
  return ceil(test/2.);
}

//ew ew ew, need to learn a better way
float rectOutlineSDF(vec2 p, vec2 dims, vec2 thickness){
  vec2 a = abs(p / dims);
  float v = max(a.x, a.y);
  float test =  step(v, dims.x) + step(v, dims.y);
  test = ceil(test/2.);
  float cutout = step(v, dims.x- thickness.y) + step(v, dims.y - thickness.x);
  test -= ceil(cutout/2.);
  return ceil(test/2.);
}
...
vec2 rot(vec2 p, float theta){
  mat2 rot1 = mat2( cos(theta), -sin(theta),
				   sin(theta), cos(theta));
  return p * rot1;
  // return vec2(0.);
}

float goodForHealth(vec2 p, float pos,  float rad, vec2 d, vec2 loopSpec){
  // float c1 = circleSDF(p + vec2(-pos, 0.), rad);
  // float r0 = rectSDF(p, d);

  vec2 a = vec2(1.0, u_res.y / u_res.x);
  vec2 toCut = a * vec2(.03);

  // float r0 = rectOutlineSDF(p, d, toCut);
  ...
  float c2 = halfLoopSDF(prot+ vec2(0.6, 0.) , loopSpec);
  // float c1 = loopSDF(p + vec2(0.6, 0.), loopSpec);
  return c1 + r0 + r1 + c2;

  // float c2 = circleSDF(p + vec2(pos, 0.), rad);
...
void main(){
  ...
  float total = 0.;
  total = goodForHealth(p, pos, rad, dims, loopSpec);

  float highlight = circleSDF(p - vec2(0.61, -0.1), 0.07);
  float leftCircle = circleSDF(p + vec2(0.6, 0.), rad - 0.05);
  float rect = rectSDF(p + vec2(0.35, 0.), vec2(0.5, 0.39));

  total += highlight + leftCircle + rect;
