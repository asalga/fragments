...
void main(){
  ...
  float ring = ringSDF(p, 2.1, 0.001);
  // if(ring < 0.){
    // ring = .0;
  // }
  // ring = max(ring, 0.);
  ...
  rects += ring;
  for(float theta = 0.; theta < TAU; theta += TAU/8.){
    p *= r2d(theta);
    vec2 dims = vec2(.5, .12);
    vec2 pos = vec2( 1.05, 0.);
    // rects += 0.001 / max(rectSDF(p+pos, dims), 0.);
  ...
  for(float b=0.;b <10.;b++){
    ...
    vec2 pp = p * r2d(b*PI/10.);
    float moveTime = t*.125;
    float time = (fract( moveTime +b/2.)*2.-1.) * sc;
    float _b = .05 / distance(pp + vec2( 0.25, 0.), vec2(time, time));
    balls += _b;
  }
