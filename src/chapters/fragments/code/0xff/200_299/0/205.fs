...
float sdScene(vec3 p, out float col){
  ...
  vec3 c = vec3(1.0);
  vec3 rp = mod(p, c)- 0.5*c;

  float b = sdBox(rp + vec3(0.5, 0.0, 0.0), vec3(1));

  float s = min(
    sdSphere(p - vec3(0.5, 0.0, 0.0), 1.),
    sdSphere(p + vec3(1.5, 0.0, 0.0), 1.)
    );

  float m = clamp( (sin(t*2.)+1.0)/2., 0.3, 1.);
  return mix(b, s,  m);
...
void main(){
  ...
  float dist = 4.;
  float x = t;
  vec3 eye = vec3(5.+cos(x), 10., sin(x)+ 5.);
  vec3 center = vec3(0, 0., 0.);
  vec3 lightPos =  vec3(0., 6., 5) + eye;
  vec3 up = vec3(0,1,0);
