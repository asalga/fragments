...
void r3dX(inout vec3 p, float a){
  vec4 p4 = vec4(p,1.);
  p4 *= r3dX(a);
  p = p4.xyz;
}

void r3dY(inout vec3 p, float a){
  vec4 p4 = vec4(p,1.);
  p4 *= r3dY(a);
  p = p4.xyz;
}
...
float sdScene(vec3 p, out float col){
  ...
  float b1 = sdBox(p + vec3(0., -1., 0.), vec3(1., 0.0001, 1.));
  float b2 = sdBox(p + vec3(0., 0., -1.), vec3(1., 1., 0.0001));
  float b3 = sdBox(p + vec3(-1., 0., 0.), vec3(0.00001, 1., 1.));

  // float b2 = sdBox(p, vec3(1));
  // r3dX(p, t/2.);
