...
float gear2(vec2 p, float r, float numTeeth, float teethHeight){
  ...
  vec2 dir = normalize(p);
  vec2 t = (dir *  min(0.0,sin(theta * numTeeth))) * teethHeight;
  return step(length(p + t), r);
}

void main(){
  vec2 a = vec2(1.0, u_res.y/u_res.x);
  vec2 p = a * ((gl_FragCoord.xy / u_res) * 2. -1.);
  float g0 = gear(vec2(0.55, 0.) + p, 0.5, 20.0, 0.2, 1.);
  float g1 = gear(vec2(-.55, 0.) + p, 0.5, 20.0, 0.2, -1.);
